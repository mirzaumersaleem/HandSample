...
async function getMainAndSubCat(parentCategories) {
  var mainCatImages = [
    "/sadaliaCats/Medicines&Treatments.png",
  ...
  return new Promise(async function(resolve) {
    ...
    for (i = 0; i < parentCategories.length; i++) {
      var subCategories = await categories1.getSubCatPromise(
        parentCategories[i].id
      ); //The execution would wait until subcategories are fetched
  ...
  products.getSubCatProd(req.query.subCategoryId, function(err, result) {
    if (err) {
      ...
      for (var i = 0; i < result.length; ++i) {
        ...
        result[i].images =
          "http://hikvisionsaudi.com/9/uploads/images/full/" + imageLink;
      }
      res.json({
        status: 200,
        data: result
    ...
    if (err) {
      ...
      if (result.length != 0) {
        ...
        for (var i = 0; i < prod.length; i++) {
          if (prod[i]!=undefined) {
         //   console.log("Offer", Object.keys(prod).length);
