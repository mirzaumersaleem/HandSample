...
class user {
    ...
    getUserAddresses(userId, callback) {
        ...
        var query = "SELECT id,first_name,address_1,city,postal_code\
                     FROM myraal_raal.addresses WHERE user_id = " + userId;
        //  INNER JOIN myraal_raal.users AS customers\
        //  ON customers.id = address.user_id\
    ...
    addUserAddress(userId, addressData, callback) {
        // var query = "INSERT INTO myraal_raal.addresses\
        //              (CustomerId, latitude, longitude, address)\
        //              VALUES (" + userId + "," + addressData.latitude + "," + addressData.longitude
        //     + "," + "\"" + addressData.addressDesc + "\"" + ")";
        var query = `INSERT INTO myraal_raal.addresses set ?`
        mySql.getConnection(function (err, connection) {
            if (err) {
                throw err;
            }
            connection.query(query, addressData, function (err, rows) {
                connection.release()
                console.log(rows);
                callback(err, rows); //Passing results to callback function
            });
