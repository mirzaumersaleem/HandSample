...
router.post('/addFriend', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        ...
        conn.query(`insert friends set ?`, favData).then(() => {
            res.json({ status: 200, message: 'Friend added Successfully', favData });
        }).catch(err => {
...
router.post('/acceptMoneyMenu', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        //console.log("name - > "+ req.body.name);
        //searching on the basis of name
        const qry = `select * from receives where receives.receiver_id = ${req.body.receiver_id} && status = 0`;
        conn.query(qry).then(users => {
            res.json({ status: 200, data: users });
...
router.post('/acceptMoney', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        ...
        //searching on the basis of name
        var status_value = 0;
        select = `select * from balances where customer_id = ${req.body.customer_id}`;
                    conn.query(select)
                    .then(result => {
            ...
            upBalance = {
                    ...
                    conn.query(`insert update_balances set ?`, upBalance)
                    .then(res => {
                    ...
                    .then(res => {
                        return Promise.using(mysql.getSqlConn(), conn => {
                            ...
                            return conn.qry(`update balances set ? where customer_id = ${req.body.customer_id}`, blnObj)
                        })
                        });
                    })
                    .then(res => {
                        return Promise.using(mysql.getSqlConn(), conn => {
                            ...
                            return conn.qry(`update receives set ? where id = ${req.body.id}`, rcvObj)
                        })
...
router.post('/rejectMoney', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        ...
        select = `select * from balances where customer_id = ${resu[0].sender_id}`;
                    conn.query(select)
                    .then(result => {
                        //res.json({ status: 200, message: 'balance added Successfully', upBalance });
            ...
            upBalance = {
                    ...
                    .then(res => {
                        ...
                        res.json({ status: 200, message: 'balance added back to senders account Successfully'});
                        });
                    })
                    .catch(err => {
                        res.json({ status: 500, message: 'Error Occured', err });
...
router.post('/addMoney', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        //console.log("name - > "+ req.body.name);
        //searching on the basis of name
        const qry = `select balance as current_balance from balances where customer_id = ${req.body.customer_id}`;
        conn.query(qry).then(users => {
            res.json({ status: 200, data: users });
        ...
        select = `select * from balances where customer_id = ${req.body.customer_id}`;
            ...
            upBalance = {
                    ...
                    .then(res => {
                        ...
                        res.json({ status: 200, message: 'Money added to account Successfully'});
                    })
                    .catch(err => {
                        res.json({ status: 500, message: 'Error Occured', err });
                    });
...
router.post('/verifyPinAddMoney', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        ...
        const qry = `select secret_password as pin from customers where customer_id = ${req.body.customer_id}`;
        conn.query(qry).then(users => {
        ...
        select = `select * from balances where customer_id = ${req.body.customer_id}`;
            ...
            upBalance = {
                    ...
                    .then(res => {
                        ...
                        res.json({ status: 200, message: 'Transaction Succeed', transaction_id: res.insertId, account_no: req.body.account_no});
                    })
                    .catch(err => {
                        res.json({ status: 500, message: 'Error Occured', err });
                    });
...
router.post('/verifyPinTransaction', (req, res) => {
    ...
    Promise.using(mysql.getSqlConn(), conn => {
        ...
        //searching on the basis of name
        var status_value = 0;
        const select = `select secret_password as pin from customers where customer_id = ${req.body.customer_id}`;
                    conn.query(select)
                    .then(result => {
                        //res.json({ status: 200, message: 'balance added Successfully', upBalance });
                    if(result[0].pin == req.body.pin){
                        const slct = `select * from update_balances where id = ${req.body.transaction_id}`;
                        conn.query(slct)
                        .then(ress => {
                            upBalance = {
                        ...
                        .then(res => {
                            return Promise.using(mysql.getSqlConn(), conn => {
                                select = `select customer_id as receiver_id from balances where account_no = ${req.body.account_number}`
                                conn.query(select).then(result => {
                                rcvObj = {
                                    receiver_id: result[0].receiver_id,
                        ...
                        .then(res => {
                            ...
                            res.json({ status: 200, message: 'Money transaction processed Successfully'});
                        })
                        .catch(err => {
                            res.json({ status: 500, message: 'Error Occured', err });
                        });
